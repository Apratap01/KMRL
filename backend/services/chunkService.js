...
export async function getChunkFromFastAPI(file) {
  try {
    const formData = new FormData();
    formData.append('file', fs.createReadStream(file.path));

    const response = await axios.post(`${FASTAPI_URL}/upload-and-build/`, formData, {
      headers: {
        ...formData.getHeaders(),
      },
    });
    ...
    throw new Error('Failed to get conversation Id from FastAPI.');
  }
}